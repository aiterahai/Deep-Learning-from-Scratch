...
* **연쇄법칙**
  ...
  역전파는 국소적인 미분을 순방향과는 반대인 오른쪽에서 왼쪽으로 전달합니다.

  이에 대한 원리적 정당성을 부여하는 것이 연쇄법칙(Chain rule)에 따른 것 입니다.

  따라서 위의 챕터에서는 연쇄법칙이 계산 그래프 상의 역전파와 같음을 보이는 것이 목표입니다.

  

  예를 들어 y = f(x)라는 계산의 역전파를 계산 그래프로 그리면 다음과 같습니다.

  ![img](https://blog.kakaocdn.net/dn/n2Og1/btq3KRNsgtP/tKtUGYq6H4YjZkDnIge521/img.png)

  위에서 보이는 것 처럼, 역전파 과정에서 순전파 일때의 y = f(x) 계산의 미분을 구하는 것 입니다.

  즉, x에 대한 y의 증분을 구한다는 의미입니다.

  역전파의 처리과정은 국소적인 미분을 상류에서 전달된 값에 곱해 앞쪽 노드로 전달하는 것 입니다.

  

  이렇게 되는 이유는 연쇄법칙으로 설명할 수 있습니다.

  예를 들어 국소적인 미분을 상류에서 전달된 값에 곱해 앞쪽 노드에 계속적으로 곱했다고 가정합니다.

  ![img](https://blog.kakaocdn.net/dn/d0bWpw/btq3TsrX6EO/EAUYo3m9DYllfBq9oLRJlk/img.png)

  그러면 x 간선 입장에서 저 값은 

  ![image-20210919201152923](Image\Chain_1.png)

  이고 연쇄 법칙에 의해

  ![image-20210919201236967](Image\Chain_2.png)

  로 정리할 수 있습니다.

  그런데, 위 식은 z를 x에 대해 미분한 것이므로 손실함수 x에 대해 미분하여 얻은 편미분 값과 동일합니다.

  

  즉, 이를 통해 수치 미분 계산이 느리다는 단점을 해결할 수 있게 됩니다.